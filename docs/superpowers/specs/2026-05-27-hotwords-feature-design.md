...
| 每词长度 | ≤10 字 / ≤30 字节 | ≤30 字节（约 10 汉字或 30 字母） |
...
HotWord:      id, word (db.String(30), 业务约束 ≤30 字节), table_id (FK → HotWordTable.id, cascade delete)
```

- DB 类型 `db.String(30)`（UTF-8 中文每字 3 字节，10 字需 30 字节）
- 业务逻辑约束每词 ≤30 字节（约 10 个汉字或 30 个英文字母）
- 删除词表时级联删除其所有热词
...
| 规则 | 说明 |
|------|------|
| 每词 ≤30 字节（约 10 汉字或 30 字母） | word 长度限制 |
...
- 底部提示行：
  - "每次只能选择一张热词词表，热词与术语合计不超过 1000 条。"
...
参考现有 `update_glossary` 机制，新增 `update_hotwords` Socket.IO 事件。采用**替换策略**——每次收到事件直接覆盖当前会话的 `hot_words_list`，而非增量追加：

```
// 前端发送（仅携带 table_id，后端从 DB 全量加载）
socket.emit('update_hotwords', { table_id: 3 })

// 后端响应
socket.emit('update_hotwords_response', { success: true, count: 12 })
     ↓
后端根据 table_id 从 DB 全量加载热词
     ↓
...
会话必须选择一张已有词表，若无可选需先新建。会话运行中在热词管理页增删改词后，JS 自动触发 `emit('update_hotwords', {table_id})` 同步到当前会话。
...
| 校验失败 | 前端即时提示（如"词超出 30 字节限制"），后端返回 400 + 错误信息 |
...
| `app/static/js/hotwords.js` | 新建 | 热词管理页面 JS |
| `app/static/js/translator.js` | 修改 | 会话配置加载热词词表、概览显示 |
